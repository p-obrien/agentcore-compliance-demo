...
How the browser reaches the runtime, and why it does not weaken isolation: a static page cannot call the assessment runtime directly (the invocation is a SigV4-signed AWS API with no CORS surface, and the assessors are Cognito identities with no AWS credentials), so a small backend Lambda relays the call. The runtime uses a CUSTOM_JWT authorizer, so the relay reaches its HTTPS invocations endpoint with the caller's bearer token rather than the SigV4 SDK call, and holds no tenant logic. It forwards three headers unchanged: `Authorization: Bearer <access>` (which the runtime's JWT authorizer validates), `X-Access-Token: <access>` (which the agent reads to verify the subject and authenticate the Gateway call, because AgentCore consumes the inbound `Authorization` for its own authorizer and does not pass it to the agent), and `X-Id-Token: <id>`. The runtime and retrieval Lambda still derive tenant from the signed claims, exactly as they do for any other client. The demo API's own IAM permits only `InvokeAgentRuntime` on the assessment runtime and `dynamodb:Query` on the audit table, so the web layer can neither reach another runtime nor write the trail. If a reviewer asks whether the web app enforces the boundary, the answer is no, and that is the point.
...
- No WAF is attached. The approval and demo APIs are API Gateway HTTP APIs (v2), which AWS WAF cannot front, and every data route already requires a Cognito JWT. As native rate control the stages set request throttling (approval `20`/`10`, demo `10`/`5` burst/rate) and write a JSON access log to `/aws/apigateway/<name_prefix>-{approval,demo}-api`. A production design that wants WAF must either front these APIs with CloudFront and attach a WebACL there, or move them to REST APIs (v1) for a regional WebACL, and add managed rule groups plus IP rate limiting. WAF on the two static-site CloudFront distributions would add nothing here: their origins are private S3 buckets serving public HTML.
